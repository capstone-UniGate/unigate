...
def test_undo_join_request_success():
    ...
    join_request = next(
        (req for req in requests_data if str(req["student"]["number"]) == other_user_number), None
    )
    assert join_request is not None, f"Join request not found. Debug data: {requests_data}"
    ...
    assert not any(req["student"]["number"] == other_user_number for req in updated_requests_data), (
