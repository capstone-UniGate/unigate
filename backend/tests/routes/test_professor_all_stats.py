...
def fetch_all_stats() -> dict:
    ...
    assert response.status_code == 200, f"Failed to fetch course stats: {response.json()}"
    return response.json()
...
def get_all_course_names() -> list[str]:
    ...
    assert response.status_code == 200, f"Failed to fetch course names: {response.json()}"
    return response.json()

def modular_test(course_name: str, expected_value: str | int, argument_to_test: str, exam_date: str = None) -> None:
    ...
    if exam_date:
        matching_group = next((group for group in course_details if group['exam_date'] == exam_date), None)
        assert matching_group, f"Exam on {exam_date} not found for course {course_name}"
    else:
        matching_group = course_details[0]

    assert argument_to_test in matching_group, f"Argument {argument_to_test} not found in course details"
